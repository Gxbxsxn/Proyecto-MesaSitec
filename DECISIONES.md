...
**e) Bootstrap importado completo (CSS) en vez de una build a medida con Sass.**
El cliente pidió explícitamente usar Bootstrap para el login. Descarté configurar un build de Sass con solo los componentes de Bootstrap que se usan (grid, forms, buttons) porque hubiera necesitado agregar `sass` como dependencia de build y configurar imports parciales, más tiempo del que quedaba disponible. Importé `bootstrap/dist/css/bootstrap.min.css` completo desde `main.ts`, después de eso nuestros propios estilos en `estilos.css` (que se cargan después en el mismo archivo) siguen ganando la cascada para las clases que ya usábamos (`.btn`, `.card`, `table`, etc.), así que el resto de la app no cambió de aspecto. El costo es un CSS más pesado de lo estrictamente necesario (~45 KB gzip) — si esto creciera, valdría la pena migrar a una build parcial de Sass.
